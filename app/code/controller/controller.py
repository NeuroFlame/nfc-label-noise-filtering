...
def _load_and_set_computation_parameters(fl_ctx: FLContext) -> None:
    """
    Load computation parameters from a file.

    Sets them in the shared context for all sites to access.

    This is a utility method that computation authors typically do not
    need to modify.
...
class LAMPController(Controller):
    ...
    def __init__(
        ...
        """
            Initiated by the Job to complete a workflow.

            :param min_clients: Minimum number of client responses required.
            :param wait_time_after_min_received: Time to wait after receiving
                                                minimum responses.
    ...
    def start_controller(self, fl_ctx: FLContext) -> None:
        """
            ...
            It assigns the aggregator component and loads computation parameters
        ...
        self.aggregator = self._engine.get_component(LAMP_AGGREGATOR_ID)
        # Load and set computation parameters for the sites
        _load_and_set_computation_parameters(fl_ctx)
        ...
        log_level = fl_ctx.get_prop('COMPUTATION_PARAMETERS')['LogLevel']
        fl_ctx.set_prop(key="log_level", value=log_level)
        self.log = NvFlareLogger(remote_name, output_path, log_level)

    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext) -> None:
        """
        Main method for defining the control flow of computations.

        This is where developers implement the main workflow. Broadcasts tasks
        to all sites, aggregates results, and broadcasts the final global
        result.
        ...
        # Broadcast the regression task and send site results to aggregator
        self._broadcast_task(
            task_name=PERFORM_LOCAL_CRF,
            data=Shareable(),
    ...
    def _accept_site_regression_result(self, client_task: ClientTask, fl_ctx: FLContext) -> bool:
        """
        Callback method that processes each site's regression result.

        Sends it to the aggregator for aggregation. Computation authors can
        override this if they need to modify how results are processed before
        aggregation, but this is optional.

        :param client_task: The task result received from a client site.
        :param fl_ctx: Federated learning context for this run.
        :return: Boolean indicating whether result was successfully accepted.
        """
    ...
    def _broadcast_task(
        ...
        """
        Broadcast a task to all client sites and wait for responses.
    ...
    def process_result_of_unknown_task(self, task: Task, fl_ctx: FLContext) -> None:
        """
        Handle results for tasks that are not explicitly recognized.
    ...
    def stop_controller(self, fl_ctx: FLContext) -> None:
        ...
        Developers can override this method for cleanup or custom logic.
