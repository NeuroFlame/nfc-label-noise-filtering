"""Shared helpers for CRF tree construction: splitting, labeling, leaf checks."""
...
class NodeValues:
    """Holds one tree node's subject ids and their ancestry labels."""

    def __init__(self):
        """Initialize empty id/label lists."""
        self.datas: np.ndarray = []
        self.labels: list = []


class Node:
    """One node of a CRF decision tree."""

    def __init__(self, value: NodeValues):
        """Wrap ``value`` as a leaf node with no children yet."""
...
def otsu4_thres(array: np.ndarray):
    ...
    for v in range(n):
        ...
        fun[v] = (
            size_n1 * (mean_n1 - agg_mean) ** 2 + size_n2 * (mean_n2 - agg_mean) ** 2
        )  # between-class variance score
...
def parallel_sort(frequency: np.ndarray) -> np.ndarray:
    """Return indices of entries tied for the top frequency.
...
def node_label(data: np.ndarray, flag: int, up_labels: list) -> int:
    """Pick this node's label, breaking frequency ties using ``flag``/``up_labels``.
...
def check_label_sequence(labels) -> int:
    """Return the length of the second run of repeated labels in the ancestry sequence."""
    n = len(labels)
    if n < 2:
        return 0
...
def check_tree_leaf(sr_tree: Node) -> np.ndarray:
    """Flatten a CRF tree into per-subject label-sequence results.
