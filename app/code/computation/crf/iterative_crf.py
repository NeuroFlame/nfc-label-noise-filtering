"""Iterative (non-recursive) construction of one CRF tree."""
...
from .crf_helpers import check_label_sequence, node_label, otsu4_thres


def build_crf_results_iter(
    data: np.ndarray, is_continuous: np.ndarray, flag: int, sd_id: int
) -> np.ndarray:
    """Build one CRF tree over ``data`` and return each subject's label-sequence score."""
